    ...
    <div className="border w-[300px] bg-black text-white h-full fixed top-0 left-0 flex flex-col justify-between z-50">
      ...
      <div>
        ...
        <div className="flex flex-col">
          <Link
            href="/admin"
            className={`h-[65px] flex items-center hover:bg-neutral-600 cursor-pointer gap-2 pl-[65px] ${isActive(
              "/admin"
            )}`}
          >
            <h1>Dashboard</h1>
      ...
      <div className="relative flex flex-col bg-gray-700">
        {dropdownOpen && (
          <div className="bg-gray-800 text-center py-2 absolute w-full bottom-14 cursor-pointer">
            <button
              onClick={handleSignOut}
              className="text-sm text-white hover:text-red-500 transition cursor-pointer"
            >
              Sign Out
            </button>
          </div>
        ...
        >
          ...
          <p className="flex-1 text-center">{session?.user?.name || "Admin"}</p>
          {dropdownOpen ? (
