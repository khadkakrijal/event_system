...
const GalleryCard: React.FC = () => {
  ...
  return (
    <div className="bg-black flex flex-col gap-12 justify-center items-center z-10 pt-11">
      {loading ? (
        <p className="text-white text-center">Loading galleries…</p>
      ) : error ? (
        <p className="text-red-400 text-center">{error}</p>
      ) : galleryEvents.length === 0 ? (
        <p className="text-white text-center py-10 text-lg">
          No galleries found at the moment.
        </p>
      ) : (
        <>
          {/* Top filter UI */}
          ...
          <div className="bg-black flex justify-between items-center md:w-[78vw]">
            ...
            <div>
              ...
              >
                ...
                >
                  <path
                    d="M14 14L0.143595 0.5L27.8564 0.5L14 14Z"
                    fill="#D9D9D9"
                  />
                </svg>
              </button>
          ...
          >
            ...
            {!loading &&
              ...
              visibleEvents.map((event) => (
                <Link
                  key={event.id}
                  href={`/event/album/${slugify(event.title)}`}
                >
